        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                if let (Some(renderer), Some(window), Some(state)) = (&mut self.renderer, &self.window, &mut self.egui_state) {
                    ...
                    // Fetch objects and process camera if we are in 3D Mode!
                    let visible_objects = if self.is_playing {
                        ...
                        Vec::new() // Render nothing in 3D if in menu
                    };

                    let raw_input = state.take_egui_input(window.as_ref());
                    
                    // Build UI only if in Menu
                    let full_output = if !self.is_playing {
                        ...
                        self.egui_ctx.run(raw_input, |ctx| {
                            egui::CentralPanel::default()
                                .frame(egui::Frame::NONE.fill(egui::Color32::from_rgb(0, 0, 180)))
                                .show(ctx, |ui| {
                        ...
                        // Empty UI run so we get valid primitives to pass to the renderer
                    ...
                    // IMPORTANT FIX: Passing the camera and visible objects to the renderer!
...
fn main() -> Result<()> {
    ...
    let mut app = EngineApp {
        ...
        is_playing: false, 
        input_state: game::world01::controls::InputState::default(),
