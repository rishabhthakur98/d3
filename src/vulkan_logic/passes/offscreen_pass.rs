// src/vulkan_logic/passes/offscreen_pass.rs
...
impl OffscreenPass {
    pub fn new(context: &VulkanContext, extent: vk::Extent2D) -> Result<Self> {
        let allocator = context.allocator.as_ref().ok_or_else(|| anyhow!("Allocator missing"))?;
        ...
        let alloc_create_info = AllocationCreateInfo { usage: MemoryUsage::Auto, ..Default::default() };
        let (color_image, color_alloc) = unsafe { allocator.create_image(&color_info, &alloc_create_info) }.map_err(|e| anyhow!("Failed to allocate offscreen color image: {}", e))?;
        ...
        let color_view = unsafe { context.device.create_image_view(&color_view_info, None) }.map_err(|e| anyhow!("Failed to allocate offscreen color view: {}", e))?;
        ...
        let (depth_image, depth_alloc) = unsafe { allocator.create_image(&depth_info, &alloc_create_info) }.map_err(|e| anyhow!("Failed to allocate offscreen depth image: {}", e))?;
        ...
        let depth_view = unsafe { context.device.create_image_view(&depth_view_info, None) }.map_err(|e| anyhow!("Failed to allocate offscreen depth view: {}", e))?;
        ...
        let render_pass = unsafe { context.device.create_render_pass(&rp_info, None) }.map_err(|e| anyhow!("Failed to create offscreen render pass: {}", e))?;
        ...
        let framebuffer = unsafe { context.device.create_framebuffer(&fb_info, None) }.map_err(|e| anyhow!("Failed to create offscreen framebuffer: {}", e))?;
        ...
        let sampler = unsafe { context.device.create_sampler(&sampler_info, None) }.map_err(|e| anyhow!("Failed to create offscreen sampler: {}", e))?;
