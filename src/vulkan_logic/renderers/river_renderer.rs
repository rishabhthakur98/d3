...
impl RiverSystem {
    pub fn new(context: &VulkanContext, render_pass: vk::RenderPass) -> Result<Self> {
        ...
        let pipeline = unsafe { context.device.create_graphics_pipelines(vk::PipelineCache::null(), &[pipeline_info], None) }
            .map_err(|e| anyhow!("Failed to create river pipeline: {:?}", e.1))?[0];
    ...
    pub fn draw(&mut self, context: &VulkanContext, cmd: vk::CommandBuffer, rivers: &[RiverConfig], view_proj: glam::Mat4, cam_pos: glam::Vec3, sun_dir: glam::Vec3, sun_color: [f32; 3], intensity: f32, time: f32) -> Result<()> {
        ...
        unsafe {
            ...
            for (i, config) in rivers.iter().enumerate().take(10) {
                ...
                let pc = RiverPushConstants { model, river_index: i as u32 };
                let pc_bytes = std::slice::from_raw_parts(&pc as *const _ as *const u8, std::mem::size_of::<RiverPushConstants>());
                context.device.cmd_push_constants(cmd, self.pipeline_layout, vk::ShaderStageFlags::VERTEX | vk::ShaderStageFlags::FRAGMENT, 0, pc_bytes);
