...
impl MasterRenderer {
    ...
    pub(crate) fn record_main_pass(
        ...
        unsafe {
            ...
            // PHASE 1: Render the 3D Scene to the HDR Offscreen Pass
            let offscreen_info = vk::RenderPassBeginInfo::default()
                .render_pass(self.offscreen_pass.render_pass).framebuffer(self.offscreen_pass.framebuffer)
                .render_area(vk::Rect2D { offset: vk::Offset2D { x: 0, y: 0 }, extent: self.swapchain_mgr.extent })
                .clear_values(&clear_values);

            self.context.device.cmd_begin_render_pass(self.sync.command_buffer, &offscreen_info, vk::SubpassContents::INLINE);
            ...
            if is_playing {
                ...
                self.skybox_system.draw(&self.context, self.sync.command_buffer, &self.swapchain_mgr.extent, skybox_config, camera_view_matrix)?;
                ...
                self.cloud_system.draw(&self.context, self.sync.command_buffer, cloud_volumes, view_proj.inverse(), camera_pos, frame_data.primary_sun_dir, frame_data.primary_sun_color, frame_data.primary_sun_intensity, time)?;
                ...
                if !frame_data.draw_calls.is_empty() {
                    ...
                    for call in &frame_data.draw_calls {
                        ...
                        self.context.device.cmd_push_constants(self.sync.command_buffer, self.pipeline.layout, vk::ShaderStageFlags::VERTEX, 0, std::slice::from_raw_parts(&pc as *const _ as *const u8, std::mem::size_of::<PushConstants>()));
                        self.context.device.cmd_draw_indexed(self.sync.command_buffer, call.index_count, 1, call.index_start, call.vertex_offset, 0);
                    }
                }

                self.river_system.draw(&self.context, self.sync.command_buffer, river_configs, view_proj, camera_pos, frame_data.primary_sun_dir, frame_data.primary_sun_color, frame_data.primary_sun_intensity, time)?;
                self.fire_system.draw(&self.context, self.sync.command_buffer, fire_emitters, view_proj, camera_view_matrix)?;
                self.smoke_system.draw(&self.context, self.sync.command_buffer, smoke_emitters, view_proj, camera_view_matrix)?;
                self.weather_system.draw(&self.context, self.sync.command_buffer, weather_emitters, view_proj, camera_view_matrix, camera_pos, time)?;
            }
            self.context.device.cmd_end_render_pass(self.sync.command_buffer);

            // PHASE 2: Post-Process to the Final Swapchain Output
            ...
            // Pull the HDR Buffer out and stylize it!
            self.post_process_system.draw(&self.context, self.sync.command_buffer, post_process_config, time)?;

            // Render UI on top of the Post-Processed image
            if !clipped_primitives.is_empty() {
                self.egui_renderer.cmd_draw(self.sync.command_buffer, self.swapchain_mgr.extent, pixels_per_point, clipped_primitives).unwrap();
            }
